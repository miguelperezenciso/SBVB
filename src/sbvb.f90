...
! contains chr features
 integer               :: nqtl=0, nsnp=0, ploidy=2 !--> ploidy 1 for  male X
...
!----------------
 type t_fgenome      
!----------------
! contains genome features 
 integer                  :: nchr=23, ntrait=1, nchip=0, nepi=0, sexchr=0, mxover=3
    ...
    !--> recombinant individual
       ...
       !--> this is for gene-dropping indivs
          ...
          if(.not.inds(ind)%hapknown .or. info%restartqtl) &
             call get_qtlori(inds(ind),gfeatures)
          do ichr=1, gfeatures%nchr; do h=1, 2
...
!-------------------
 type(t_info)    :: info
 type(t_fgenome) :: gf
 integer         :: iopar=87, ios, ichr, n, isnp, ichip
 ...
 do
    ...
    case('GWASFILE') !--> outfile with mkr data, either chip or sequence
      ...
      info%gwas=.true.
    case ('NBASE')   !--> nind which genotypes are read from STDIN (nori=2*nbase)
...
! reads founder genotypes
 ...
 maxnchr = gf%nchr
 allocate(temp_pos(maxn), temp(maxn,2*nbase), f(maxn))
 ...
 ntot=0
 do
    read(*,*,iostat=ios) new_chr, bpos, gt(1:2*nbase)
    ...
    i=i+1
    temp_pos(i)=bpos
    temp(i,1:2*nbase)=gt(1:2*nbase)
    f(i)=0.5*count(gt==1)/nbase
    if(mod(i,1000000)==0) print*, i,'markers read'
    if(i==maxn) then
       maxn0 = maxn
       maxn  = maxn + step
       allocate(tp(maxn), t(maxn,2*nbase), tf(maxn))
...
!-------------------
! read epistatsis file and assigns epistatic pair structure
 type(t_fgenome) :: gf
 type(t_info)    :: info
...
!--------------------
 ...
 nind=size(pop)
 ntrait=gf%ntrait
 allocate (ichunk(nind,2),  class(nind), gt(nind), frq(maxclass+1), alpha(maxclass+1,ntrait) )
 do ind=1,nind
    ...
    do iqtl=1, gf%fchr(ichr)%nqtl
       ...
       do it=1, ntrait
          ...
          do i=1, maxclass         
             n = count(class==i)
             if(n>0) then
                frq(1+i) = 0.5*sum(pack(gt,class==i))/n
...
! sample qtl effects following distributions and ve given h2
 ...
 integer               :: i, iclass, ind, ichr, iqtl, nqtl, ntot, g, h, n, nbase, isnp, &
                          ipair, iori, ntrait, iepi, it, delta(0:2)=(/0,1,0/), iope=333
    ...
    do ind=1, nbase
       ...
       do ichr=1, gf%nchr
          do iqtl=1, gf%fchr(ichr)%nqtl
             ...
             enddo
             tbv(ind) = tbv(ind) + (g-1)    * gf%fchr(ichr)%qtladd(iqtl,it) &
                                 + delta(g) * gf%fchr(ichr)%qtldom(iqtl,it)
             tba(ind) = tba(ind) + (g-1)    * gf%fchr(ichr)%qtladd(iqtl,it)
          enddo
       enddo
     enddo
...
! read mapfile and assigns rec chunks by chr
 ...
 !--> mapfile exists
    ...
    do ichr=1, gf%nchr
       ...
       !--> sex chromosome
       if(gf%sexchr==ichr) gf%fchr(ichr)%ploidy=1
    enddo
    call system('rm '//temp)
 ...
 !--> assign xover probs
 do ichr=1, gf%nchr
    allocate(gf%fchr(ichr)%pxover(2,gf%mxover))
    do h=1, 2
       do i=1, gf%mxover
          gf%fchr(ichr)%pxover(h,i) = ppoisson(i,gf%fchr(ichr)%lengthM(h))
       enddo
...
! ULL: I do not copy id, class and parents as causes error
 ...
 if(.not.associated(inew%g%chr)) allocate(inew%g%chr(inew%g%nchr,2))
...
! initializes chromosome type
 ...
 nx   = 1
 ind%g%nchr = nchr
 if(.not.associated(ind%g%chr)) allocate(ind%g%chr(nchr,2))
 ...
 !--> if any sex chr, copies onto dummy male X
 if(gf%sexchr>0 .and. ind%sex==1) then
    ichr = gf%sexchr
    if(present(iorig)) ind%g%chr(ichr,1)%ori(:) = ind%g%chr(ichr,2)%ori(:)
    !if(gf%fchr(gf%sexchr)%nqtl>0) ind%g%chr(ichr,1)%qtlori(:) = ind%g%chr(ichr,2)%qtlori(:)
 endif
!--------------
 end subroutine
!--------------
...
!---------------
 ...
 do ichr=1, gf%nchr
    !--> dam's chromosome 
    call mpeiosic(ind_son, ind_dam, ichr, 2, gf)
    ...
    if(gf%sexchr==ichr .and. ind_son%sex==1) then
       ...
       call mpeiosic(ind_son, ind_sire,  ichr, 1, gf)
    endif
 enddo
!--------------
 end subroutine
...
!-------------------
 subroutine mpeiosic (offspring, parent, ichr, h, gf)
!-------------------
! considers min xover 1 and max 3
! generates a recombinant gamete, genome is used to get ipos of new recombinant events
 ...
 nx1=parent%g%chr(ichr,1)%nx
 nx2=parent%g%chr(ichr,2)%nx
 ...
 if(gf%fchr(ichr)%lengthM(h)>0.0) then !--> btw 1 and 3 xovers
    call random_loaded(nxc, gf%fchr(ichr)%pxover(h,:))
 ...
 allocate(xcpos(nxc))
 call get_xcpos(xcpos, gf%fchr(ichr)%mapper(h,:), gf%fchr(ichr)%mapbp)
 ...
 temp=0
 temp(1:nx1,1)           = parent%g%chr(ichr,1)%pos(1:nx1)
 temp(1:nx1,2)           = parent%g%chr(ichr,1)%ori(1:nx1)
 temp(nx1+1:nx1+nx2-1,1) = parent%g%chr(ichr,2)%pos(1:nx2-1)
 temp(nx1+1:nx1+nx2-1,3) = parent%g%chr(ichr,2)%ori(1:nx2-1)
 temp(nx1,3)             = parent%g%chr(ichr,2)%ori(nx2) !--> last pos shared chunk
...
! allocate epistasis genotypes
 integer, parameter :: delta(0:2)=(/0,1,0/)
 ...
 do ichr=1, gf%nchr
    do iqtl=1, gf%fchr(ichr)%nqtl
       ...
       do h=1, 2
          ...
          g    = g + ff(iori)%g%chr(ichr,1)%gt(isnp)
       enddo
       indiv%tbv = indiv%tbv + (g-1)    * gf%fchr(ichr)%qtladd(iqtl,:)
...
!----------------------
 subroutine dogenotypes_v0 (pop, gf, ff, datos, ichip)
!----------------------
...
! chip contains markers to be genotyped, all genotyped if absent
 ...
 type(t_fgenome)        :: gf  
 type(t_info)           :: datos  
 integer                :: nind, nsnp, ichr, isnp, icsnp, ipos, ind, ifirst, h, ori, iom=21, iogrm=22, &
                           i1, i2, nhtrg, nthr, i, iogwas=652, maxclass, ksnp
 integer(i8)            :: bpos
 real                   :: freq, t1, t2
 ...
 ksnp = 0
 allocate(ichunk(nind,2), gt(nind,2))
 do ichr=1, gf%nchr
    ...
    do icsnp=1, nsnp
       ...
       if(datos%gwas) then
          do i=1, maxclass
             ...
             f(i) = 0.5 * sum(pack(g(:,isnp),class==i)) / count(class==i)
          enddo
          write(iogwas,'(i3,i12,100f10.4)') ichr, bpos, real(f(:)), real(b(:)), real(b(:)/sb(:))
       endif
 
       if(datos%printgrm) then
          grm_d = grm_d + 2.*freq*(1.-freq)
          g(:,isnp) = g(:,isnp) - 2.*freq
       endif
...
! chip contains markers to be genotyped, all genotyped if absent
 ...
 integer                :: nind, nsnp, ichr, isnp, icsnp, ipos, ind, h, ori, iom=21, iogrm=22, &
                           i1, i2, nhtrg, nthr, i, iogwas=652, maxclass, ksnp, ifirst
 integer(i8)            :: bpos
 real                   :: freq, t1, t2
 real(r8)               :: grm_d, one=1.d0, vx, cxy
 integer, allocatable   :: ichunk(:,:), gt(:,:), class(:), nclass(:)
...
!--------------------
! prints both qtl and hap structure
 type(t_indiv)   :: pop(:)
 type(t_fgenome) :: gf
 character       :: hapfile*(*)
 ...
 do ind=1, size(pop)
    ...
    do ichr=1, pop(ind)%g%nchr
       ...
       do iq=1, size(pop(ind)%g%chr(ichr,1)%qtlori)
          write(ioh,*) (pop(ind)%g%chr(ichr,h)%qtlori(iq), h=1,2)
       enddo
    enddo
 enddo
 close(ioh)
...
! reads haplotypes and allocates memory for each ind
 ...
 do
    ...
    do ichr=1, gf%nchr
       ...
       if(.not.restartqtl) then
          do iq=1, nqtl(ichr)
             read(ioh,*) (pop(ind)%g%chr(ichr,h)%qtlori(iq), h=1,2)
          enddo
       else
          do iq=1, nqtl(ichr)
             read(ioh,*) 
